...
export const WeeklyAnalytics: React.FC<WeeklyAnalyticsProps> = ({
  ...
  return (
    <div className="bg-surface-lowest border border-outline-subtle rounded-lg p-6 shadow-card space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-outline-subtle">
      ...
      <div
        className="grid grid-cols-7 gap-2 pt-2 items-end h-32"
        role="region"
        aria-label="Weekly focus distribution chart"
      >
        {days.map((day, idx) => {
          ...
          return (
            <div
              key={day.dateStr}
              role="img"
              aria-label={`${day.dateStr}: ${day.focusMins} minutes focus, ${day.completedTasks} tasks completed`}
